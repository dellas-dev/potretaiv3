...
export default {
  async fetch(request, env, _ctx) {
    ...
    try {
      ...
      // ── Route: Generate base couple photo (flux-schnell) ───────────
      if (path === '/generate-base') {
        const body = await request.json();
        if (!body.prompt) return jsonResponse({ error: 'prompt required' }, 400, origin);

        const count = parseInt(body.count) || 4;
        const batchSize = 2; // 2 images per call to avoid timeout
        ...
        const results = await Promise.allSettled(
          Array.from({ length: batches }, () => callBatch())
        );
        ...
        if (!urls.length) return jsonResponse({ error: 'Generate base gagal' }, 502, origin);
        return jsonResponse({ success: true, urls }, 200, origin);
      }

      // ── Route: Submit faceswap to Easel queue (async) ─────────────
      if (path === '/faceswap-submit') {
        const body = await request.json();
        if (!body.target_image) return jsonResponse({ error: 'target_image required' }, 400, origin);
        if (!body.male_url)     return jsonResponse({ error: 'male_url required' }, 400, origin);
        if (!body.female_url)   return jsonResponse({ error: 'female_url required' }, 400, origin);

        const payload = {
          face_image_0: body.male_url,
          gender_0: 'male',
          face_image_1: body.female_url,
          gender_1: 'female',
        ...
        // Submit to QUEUE (not sync) — returns immediately with request_id
        const res = await fetch('https://queue.fal.run/easel-ai/advanced-face-swap', {
          method: 'POST',
          headers: {
            'Authorization': `Key ${apiKey}`,
            'Content-Type': 'application/json',
        ...
        if (!res.ok) {
          ...
          console.error('[PotretAI] faceswap-submit error:', res.status, err);
          return jsonResponse({ error: 'Submit faceswap gagal', detail: err }, 502, origin);
        }

        const data = await res.json();
        console.log('[PotretAI] faceswap submitted:', data.request_id);
        return jsonResponse({
          success: true,
      ...
      // ── Route: Poll faceswap result ───────────────────────────────
      if (path === '/faceswap-poll') {
        const body = await request.json();
        if (!body.request_id) return jsonResponse({ error: 'request_id required' }, 400, origin);

        const res = await fetch(
          `https://queue.fal.run/easel-ai/advanced-face-swap/requests/${body.request_id}/status`,
          {
        ...
        if (!res.ok) {
          return jsonResponse({ status: 'error' }, 502, origin);
        }

        const data = await res.json();
        console.log('[PotretAI] faceswap poll status:', data.status,
          '| keys:', Object.keys(data || {}).join(','));

        if (data.status === 'COMPLETED') {
          // Get the actual result from response_url
          const resultRes = await fetch(
            `https://queue.fal.run/easel-ai/advanced-face-swap/requests/${body.request_id}`,
            { headers: { 'Authorization': `Key ${apiKey}` } }
          );
          const result = await resultRes.json().catch(() => ({}));
          console.log('[PotretAI] faceswap result keys:', Object.keys(result || {}).join(','));
          const imgUrl = result?.image?.url
            || result?.output?.image?.url
            || result?.images?.[0]?.url
            || null;
          return jsonResponse({ status: 'completed', url: imgUrl }, 200, origin);
        }

        if (data.status === 'FAILED') {
          console.error('[PotretAI] faceswap FAILED:', JSON.stringify(data).slice(0, 300));
          return jsonResponse({ status: 'error' }, 200, origin);
        }

        // IN_QUEUE or IN_PROGRESS
        return jsonResponse({ status: data.status || 'pending' }, 200, origin);
      }

      // ── Route: Couple face swap (Easel AI — 2 faces at once) ──────
      if (path === '/faceswap-couple') {
        const body = await request.json();
        if (!body.target_image) return jsonResponse({ error: 'target_image required' }, 400, origin);
        if (!body.male_url)     return jsonResponse({ error: 'male_url required' }, 400, origin);
        if (!body.female_url)   return jsonResponse({ error: 'female_url required' }, 400, origin);
        ...
        const res = await fetch('https://fal.run/easel-ai/advanced-face-swap', {
          method: 'POST',
          headers: { 'Authorization': `Key ${apiKey}`, 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          console.error('[PotretAI Worker] faceswap error:', res.status,
            JSON.stringify(err));
          return jsonResponse({
            success: true,
            url: body.target_image,
            fallback: true,
            reason: `Easel ${res.status}: ${err?.detail || err?.message || 'unknown'}`,
          }, 200, origin);
        }
        ...
        if (!imgUrl) {
          console.error('[PotretAI Worker] faceswap no image in response:',
            JSON.stringify(data));
          return jsonResponse({
            success: true,
            url: body.target_image,
            fallback: true,
            reason: 'no image url in Easel response',
          }, 200, origin);
        }

        return jsonResponse({ success: true, url: imgUrl }, 200, origin);
      }
